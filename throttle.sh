...
iptables  -A POSTROUTING -t nat --out-interface eth0 -j MASQUERADE
iptables -A FORWARD  -i eth0 -o ${VETH_NAME}_ext -j ACCEPT 
iptables -A FORWARD -i ${VETH_NAME}_ext -o eth0 -j ACCEPT
...
cleanup(){
iptables -D POSTROUTING -t nat --out-interface eth0 -j MASQUERADE
iptables -D FORWARD  -i eth0 -o ${VETH_NAME}_ext -j ACCEPT 
iptables -D FORWARD -i ${VETH_NAME}_ext -o eth0 -j ACCEPT
